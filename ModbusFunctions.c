/**********************************************************************
* Code : Modbus Fuctions File
* Devices: TMS320F28379D
* Author : Ramanpreet Kaur and Shikha Singhal
**********************************************************************/
...
float32 Modbus_ReadByte(Uint16 slaveAddr, Uint16 funCode, Uint16 dataAddr, Uint16 regLen)
{
    ...
    for(i = 0; i < modbusTxLen; i++)                                //Request Data from Energy Meter
    {
        ...
        ScicRegs.SCITXBUF.bit.TXDT = Modbus_TxData[i];
        count++;
        if(count == 65535)
        {
            count=0;
    ...
    while(1)
    {
        while(ScicRegs.SCIFFRX.bit.RXFFINT == 0)
        {
            ModbusDataSend = ModbusDataSend + 1.0;
            DelayUs(1);

            if(ModbusDataSend > 1000000.0)
            {
                ModbusDataSend = 0.0;
                ModbusFailure = 1;
                break;
        ...
        Modbus_RxData[i]  = ScicRegs.SCIRXBUF.bit.SAR;
        // clear flags start
